...
//                return "AspectJ weaver version " + weaver.getVersion().toString() + " OK!";
    ...
    public IStatus changeWeavingState(boolean becomeEnabled) {
        ...
        if (success.getSeverity() >= IStatus.ERROR || success2.getSeverity() >= IStatus.ERROR || success3.getSeverity() >= IStatus.ERROR) {
            return new MultiStatus(ArgusJDTWeavingPlugin.ID, IStatus.ERROR, 
            ...
            return new MultiStatus(ArgusJDTWeavingPlugin.ID, IStatus.WARNING, 
...
//                JDTWeavingPreferences.setAskToReindex(true);
            ...
            return new MultiStatus(ArgusJDTWeavingPlugin.ID, IStatus.OK, 
    ...
    private IStatus changeSimpleConfiguratorManipulator(boolean becomeEnabled) {
        ...
        try {
            bundleContext = Platform.getBundle(ArgusJDTWeavingPlugin.ID).getBundleContext();
        } catch (Exception e) {
            return new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, "Cannot get bundleContext", e);
        }
        if (bundleContext == null) {
            return new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, "Cannot get bundleContext", new Exception());
        }
        ...
        try {
            ...
            if (weavingInfo == null) {
                return new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, "Could not find equinox aspects bundle", new Exception());
            }
            ...
            return new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, "Cannot load configuration", e);
        }
    }
    ...
    private IStatus getInstalledBundleInformation() {
        ...
        return new Status(IStatus.INFO, ArgusJDTWeavingPlugin.ID, sb.toString());
    }
    ...
    private IStatus changeAutoStartupAspectsBundle(boolean becomeEnabled) {
        ...
        if (allEABundles == null || allEABundles.length == 0) {
            return new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, "Could not find org.eclipse.equinox.weaving.aspectj" +
        ...
        try {
            ...
            return new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, "Error occurred in setting org.eclipse.equinox.weaving.aspectj to autostart" +
                    " so weaving service cannot be " + 
        ...
        try {
            ...
            if (f.canWrite()) {
                ...
                if (becomeEnabled == currentConfigStateIsWeaving()) {
                    ...
                    success = new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, "Could not add or remove org.eclipse.equinox.weaving.hook as a framework adaptor.");
                }
            } else {
                // cannot write to file...most likely this is because the config.ini 
                // is in a global location that is read-only.
                success = new Status(IStatus.WARNING, ArgusJDTWeavingPlugin.ID, "Could not add " +
                		"'osgi.framework.extensions=org.eclipse.equinox.weaving.hook'\n " +
                		"to the config.ini because the file is read-only.  Weaving may not be enabled.");
            }
        } catch (Exception e) {
            success = new Status(IStatus.ERROR, ArgusJDTWeavingPlugin.ID, e
                    .getMessage(), e);
        }
        return success;
    }
