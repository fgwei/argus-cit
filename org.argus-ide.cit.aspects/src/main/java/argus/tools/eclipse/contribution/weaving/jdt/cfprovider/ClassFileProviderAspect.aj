...
public privileged aspect ClassFileProviderAspect {
  ...
  boolean around(Viewer viewer, Object parentElement, Object element) :
    ...
    if (element instanceof IJawaClassFile) {
  ...
  String around(BinaryType bt) :
    ...
    if (parent instanceof IJawaClassFile) {
      return ((IJawaClassFile)parent).getSourceFileName();
    }
    else
      return proceed(bt);
  }
  ...
  IJavaElement around(JavaElement je, int ancestorType) :
    ...
    while (element != null) {
      if (element.getElementType() == ancestorType)
        return element;
      else if (element instanceof IJawaClassFile && ancestorType == IJavaElement.COMPILATION_UNIT)
