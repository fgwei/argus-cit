...
object ArgusStackFrame {

  def apply(thread: ArgusThread, stackFrame: StackFrame, index: Int = /*top frame*/ 0 ): ArgusStackFrame = {
    new ArgusStackFrame(thread, stackFrame, index)
  }
...
/**
 * A stack frame in the Argus debug model.
 ...
 */
class ArgusStackFrame private (val thread: ArgusThread, @volatile var stackFrame: StackFrame, val index: Int)
  extends ArgusDebugElement(thread.getDebugTarget) with IStackFrame with IDropToFrame {
  import ArgusStackFrame._
  ...
  private lazy val variables: Seq[ArgusVariable] = {
    (safeStackFrameCalls(Nil) or wrapJDIException("Exception while retrieving stack frame's visible variables")) {
      ...
      val visibleVariables = {
        (Exception.handling(classOf[AbsentInformationException]) by (_ => Seq.empty)) {
          stackFrame.visibleVariables.asScala.map(new ArgusLocalVariable(_, this))
        }
      }
      ...
      if (currentMethod.isNative || currentMethod.isStatic) {
        ...
        new ArgusThisVariable(stackFrame.thisObject, this) +: visibleVariables
      }
    }
  }
  ...
  /** Set the current stack frame to `newStackFrame`. The `ArgusStackFrame.variables` don't need
    *  to be recomputed because a variable (i.e., a `ArgusLocalVariable`) always uses the latest
    *  stack frame to compute its value, as it can be checked by looking at the implementation of
    *  `ArgusLocalVariable.getValue`
    */
