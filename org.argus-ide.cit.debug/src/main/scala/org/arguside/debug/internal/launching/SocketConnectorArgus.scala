 ...
 */
trait SocketConnectorArgus extends IVMConnector {
      ...
      case (key, value) =>
        p.get(key) match {
          case Some(v: String) =>
            value.setValue(v)
          case _ =>
            throw ArgusDebugPlugin.wrapInCoreException("Unable to initialize connection, argument '%s' is not available".format(key), null)
        }
    }
