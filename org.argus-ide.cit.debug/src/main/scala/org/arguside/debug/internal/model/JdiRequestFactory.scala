...
import org.arguside.debug.internal.JDIUtil
...
object JdiRequestFactory {
   ...
   */
  def createMethodEntryBreakpoint(method: Method, thread: ArgusThread): BreakpointRequest = {
   ...
   */
  def createStepRequest(size: Int, depth: Int, thread: ArgusThread): StepRequest = {
   ...
   */
  def createClassPrepareRequest(typeNamePattern: String, debugTarget: ArgusDebugTarget): ClassPrepareRequest = {
   ...
   */
  def createBreakpointRequest(referenceType: ReferenceType, lineNumber: Int, debugTarget: ArgusDebugTarget, suspendPolicy: Int): Option[BreakpointRequest] = {
