...
import org.arguside.debug.internal.model.ArgusDebugTarget.ReplaceClasses
...
private[internal] object ArgusHotCodeReplaceManager {
  ...
  def create(debugTargetCompanionActor: BaseDebuggerActor): Option[ArgusHotCodeReplaceManager] = {
    if (HotCodeReplacePreferences.hcrEnabled)
      Some(new ArgusHotCodeReplaceManager(debugTargetCompanionActor))
    else
      None
  }
}
 ...
 */
class ArgusHotCodeReplaceManager private (debugTargetCompanionActor: BaseDebuggerActor) extends IResourceChangeListener {
...
private[internal] trait HotCodeReplaceExecutor extends Publisher[HCRResult] with HasLogger {
  ...
  protected val debugTarget: ArgusDebugTarget
  ...
  private def doHotCodeReplace(launchName: String, typesToReplace: Seq[ClassFileResource]): Unit = {
    try {
      ...
      updateArgusDebugEnv(typesToReplace)
  ...
  private def updateArgusDebugEnv(changedClasses: Seq[ClassFileResource]): Unit = {
