 ...
 */
class SocketListenConnectorArgus extends IVMConnector with SocketConnectorArgus {
  ...
  override def connector(): ListeningConnector = {
    ...
    Bootstrap.virtualMachineManager().listeningConnectors().asScala.find(_.name() == SocketListenName).getOrElse(
        throw ArgusDebugPlugin.wrapInCoreException("Unable to find JDI ListeningConnector", null))
  }
  ...
  override val getIdentifier: String = ArgusDebugPlugin.id + ".socketListenConnector"
...
class ListenForConnectionProcess private (launch: ILaunch, port: Int) extends IProcess {
  ...
  def getAdapter[T](adapter: Class[T]): T = null.asInstanceOf[T]
...
class ListenForConnectionJob(launch: ILaunch, process: ListenForConnectionProcess, connector: ListeningConnector, arguments: JMap[String, Connector.Argument]) extends Job("Scala debugger remote connection listener") {
  ...
  override def run(monitor: IProgressMonitor): IStatus = {
    try {
      ...
      ArgusDebugTarget(virtualMachine, launch, null, true, allowTerminate(launch))
      ...
      case e: IOException =>
        ...
        ArgusDebugPlugin.wrapInErrorStatus("Problem while waiting to receive connection", e)
      case e: Exception =>
        connectionFailed("Unexpected problem while waiting to receive connection. See log for more details")
        ArgusDebugPlugin.wrapInErrorStatus("Unexpected problem while waiting to receive connection", e)
    }
  }
