...
import org.arguside.debug.internal.ArgusDebugger.modelProvider
...
/**
 * Base class for debug elements in the Argus debug model
 * This class is thread safe.
 */
abstract class ArgusDebugElement(debugTarget: ArgusDebugTarget) extends DebugElement(debugTarget) with ITerminate with HasLogger {
  ...
  override def getDebugTarget: ArgusDebugTarget = debugTarget

  // Members declared in org.eclipse.debug.core.model.IDebugElement

  // `ArgusDebugPlugin.id` calls some osgi and does classloading and we do not want to do it in tree view.
  override lazy val getModelIdentifier: String = ArgusDebugPlugin.id
  ...
  private def targetRequestFailed(message: String, t: Throwable): Nothing = {
    if (t == null || t.getClass().getName().startsWith("com.sun.jdi") || t.isInstanceOf[TimeoutException])
      throw new DebugException(new Status(IStatus.ERROR, ArgusDebugPlugin.id, DebugException.TARGET_REQUEST_FAILED, message, t))
    else
      throw t
  }
}
...
trait HasFieldValue {
   ...
   */
  def fieldValue(fieldName: String): ArgusValue = wrapJDIException("Exception while retrieving field " + fieldName + " in reference type") {
    ...
    ArgusValue(jdiFieldValue(field), getDebugTarget)
  }
}
...
trait HasMethodInvocation {
   ...
   */
  protected[model] def jdiInvokeMethod(method: Method, thread: ArgusThread, args: Value*): Value
   ...
   *  @throws DebugException
   */
  def invokeMethod(methodName: String, thread: ArgusThread, args: ArgusValue*): ArgusValue = {
    wrapJDIException("Exception while retrieving method " + methodName + " in reference type") {
      ...
      methods.size match {
        ...
        case 1 =>
          ArgusValue(jdiInvokeMethod(methods.get(0), thread, args.map(_.underlying): _*), getDebugTarget)
        case _ =>
   ...
   *  @throws DebugException
   */
  def invokeMethod(methodName: String, methodSignature: String, thread: ArgusThread, args: ArgusValue*): ArgusValue = {
    wrapJDIException("Exception while retrieving method " + methodName + " in reference type") {
      ...
      ArgusValue(jdiInvokeMethod(method, thread, args.map(_.underlying): _*), getDebugTarget)
    }
  }
}