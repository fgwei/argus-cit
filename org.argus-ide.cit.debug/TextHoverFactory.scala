package org.arguside.debug.internal.editor
...
import org.arguside.core.compiler.IScalaPresentationCompiler.Implicits.RichResponse
...
import org.arguside.debug.internal.model.ScalaThisVariable
import org.arguside.ui.editor.extensionpoints.{ TextHoverFactory => TextHoverFactoryInterface }
import org.arguside.ui.internal.editor.hover.ScalaHoverImpl
  ...
  def createFor(scu: ScalaCompilationUnit): ITextHover = new ScalaHoverImpl with ITextHoverExtension with ITextHoverExtension2 {
    ...
    override def getHoverInfo2(viewer: ITextViewer, region: IRegion): AnyRef = {
      val hoverInfo = for {
        ...
        for {
          t <- resp.getOption()
          stackFrame <- Option(ArgusDebugger.currentStackFrame)
...
object StackFrameVariableOfTreeFinder {
  def find(src: SourceFile, compiler: IJawaPresentationCompiler, stackFrame: ArgusStackFrame)(t: compiler.Tree): Option[IVariable] = { import compiler.{Try => _, _} 
    // ---------------- HELPERS ---------------------------------
    /////////////////////////////////////////////////////////////
