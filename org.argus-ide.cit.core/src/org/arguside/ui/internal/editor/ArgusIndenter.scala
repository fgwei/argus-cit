...
import org.arguside.core.internal.formatter.FormatterPreferences
...
class JdtPreferenceProvider(val project: IJavaProject) extends PreferenceProvider {
  ...
  def updateCache(): Unit = {
    ...
    put(ArgusIndenter.TAB_SIZE, indentSpaces)
    put(ArgusIndenter.INDENT_SIZE, indentSpaces)
    put(ArgusIndenter.INDENT_WITH_TABS, indentWithTabs)
...
/**
 * Holder for various constants used by the Argus indenter
 */
object ArgusIndenter {
...
/**
 * Indenter for the Argus UI. Based largely on
...
class ArgusIndenter(
  ...
  private def prefUseTabs = preferencesProvider.getBoolean(ArgusIndenter.INDENT_WITH_TABS)

  private def prefTabChar = if (prefUseTabs) JavaCore.TAB else JavaCore.SPACE

  private def prefTabSize = preferencesProvider.getInt(ArgusIndenter.TAB_SIZE)

  private def prefIndentationSize = preferencesProvider.getInt(ArgusIndenter.INDENT_SIZE)
