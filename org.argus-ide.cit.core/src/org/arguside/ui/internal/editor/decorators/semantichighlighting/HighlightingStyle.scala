...
object HighlightingStyle {
  def apply(preferences: Preferences, symbolType: SymbolTypes.SymbolType): HighlightingStyle = {
    ...
    HighlightingStyle(syntaxClass.getTextAttribute(preferences.store), enabled, ArgusSyntaxClasses.DEFAULT.getTextAttribute(preferences.store), deprecation, stringInterpolation)
  }

  def symbolTypeToSyntaxClass(symbolType: SymbolTypes.SymbolType): ArgusSyntaxClass = {
    symbolType match {
