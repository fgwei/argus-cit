package org.arguside.ui.internal.migration
...
import org.arguside.ui.internal.wizards.FileCreatorMapping
import org.arguside.ui.internal.wizards.ScalaWizardElement
import org.arguside.util.internal.ReflectAccess
...
class RegistryExtender extends AnyRef with HasLogger {
   ...
   * Scala IDE, which are of type [[org.arguside.ui.wizards.FileCreator]].
   *
  ...
  private def injectWizardElements(reg: AbstractExtensionWizardRegistry): Try[Unit] =
    ReflectAccess(reg) apply { ra =>
      ...
      Option(elems.findCategory(ScalaWizardCategory)) foreach { category =>
        ...
        val configElem = configElems.find(_.getAttribute("id") == "org.arguside.ui.wizards.scalaCreator")
