...
import org.arguside.util.eclipse.EclipseUtils.RichWorkbench
...
class MigrationPreferenceInitializer extends AbstractPreferenceInitializer with HasLogger {
  ...
  override def initializeDefaultPreferences(): Unit = {
    ...
    if (!IArgusPlugin().headlessMode) {
      copyKeyBindings()
      activateNewWizardShortcut(CitConstants.ArgusPerspectiveId, CitConstants.ScalaFileCreatorWizId)
    }
  }
  ...
  private def activateNewWizardShortcut(perspId: String, wizardId: String) = {
    ...
    val prefId = s"org.arguside.ui.wizardActivated_$wizardId"
