...
import org.arguside.ui.internal.editor.SaveActionExtensions
...
class SaveActionsPreferencePage extends PreferencePage with IWorkbenchPreferencePage {
  ...
  private val prefStore = IArgusPlugin().getPreferenceStore()
  ...
  private def createPreviewer(parent: Composite)(f: IDocument => Unit): Control = {
    val previewer = new PreviewerFactory(ArgusPreviewerFactoryConfiguration).createPreviewer(parent, prefStore, "")
...
class SaveActionsPreferenceInitializer extends AbstractPreferenceInitializer {
  ...
  override def initializeDefaultPreferences(): Unit = {
    SaveActionExtensions.saveActionSettings foreach { s =>
      IArgusPlugin().getPreferenceStore().setDefault(s.id, false)
    }
    IArgusPlugin().getPreferenceStore().setDefault(SaveActionExtensions.SaveActionTimeoutId, SaveActionDefaultTimeout)
  }
}
