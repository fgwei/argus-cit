...
import org.arguside.util.TextEdit
...
object EclipseUtils extends HasLogger {
  ...
  private[arguside] implicit class RichWorkbench(w: IWorkbenchWindow) {
  ...
  private[arguside] def asEclipseTextEdit(edit: TextEdit): EclipseTextEdit =
   ...
   *  @return The job
   */
  private[arguside] def prepareJob(name: String, rule: ISchedulingRule = null, priority: Int = Job.LONG)(f: IProgressMonitor => IStatus): Job = {
