...
/** Add entries to the JDT index. This class traverses an *unattributed* Argus AST. This
 ...
 *  because the Argus typechecker modifies the tree. One prime example is annotations, which
 ...
 */
trait ArgusIndexBuilder extends HasLogger { self: ArgusPresentationCompiler =>

  class IndexBuilderTraverser(indexer : ArgusSourceIndexer) extends Traverser {
