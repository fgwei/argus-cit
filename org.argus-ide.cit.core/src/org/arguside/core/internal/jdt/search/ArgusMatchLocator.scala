...
import org.arguside.core.internal.jdt.model._

//FIXME should report all and let matcher to the selection OR only report matcher interest (pre select by type) OR ...

trait ArgusMatchLocator { self: ArgusPresentationCompiler =>
  val OneStar = Array('*')

  def MatchLocator(scu: ArgusCompilationUnit, matchLocator: MatchLocator, possibleMatch: PossibleMatch): Traverser =
     MatchLocator(scu, matchLocator, matchLocator.pattern, possibleMatch)

  def MatchLocator(scu: ArgusCompilationUnit, matchLocator: MatchLocator,
