...
import org.arguside.logging.Logger
 ...
 *  in the Eclipse Logger. Indeed, the call to `ArgusPlugin.plugin.getLog()` isn't thread-safe, but
...
object EclipseLogger extends Logger {
  private val pluginLogger: ILog = IArgusPlugin().getLog()
  ...
  private def logInUiThread(severity: Int, message: Any, exception: Throwable): Unit = {
    val status = new Status(severity, IArgusPlugin().getBundle.getSymbolicName, message.toString, exception)
    if (IArgusPlugin().headlessMode) pluginLogger.log(status)
