...
object Nature {
  ...
  def removeScalaLib(jp: IJavaProject) {
    val scalaLibPath = Path.fromPortableString(CitConstants.ScalaLibContId)
  ...
  def addScalaLibAndSave(project: IProject) {
    ...
    val scalaLibEntry = JavaCore.newContainerEntry(Path.fromPortableString(CitConstants.ScalaLibContId))
...
class Nature extends IProjectNature {
  ...
  override def configure() {
    ...
    updateBuilders(project, List(JavaCore.BUILDER_ID), CitConstants.BuilderId)
  ...
  override def deconfigure() {
    ...
    updateBuilders(project, List(CitConstants.BuilderId), JavaCore.BUILDER_ID)
