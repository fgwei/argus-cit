...
import org.arguside.ui.internal.editor.ArgusEditor
...
final class PresentationCompilerProxy(name: String, initializeSettings: () => Settings) extends IPresentationCompilerProxy
  ...
  /** Current 'live' instance of the presentation compiler.
    ...
    */
  private var pc: ArgusPresentationCompiler = null
  ...
  override def apply[U](op: IArgusPresentationCompiler => U): Option[U] =
    internal(op)

  /** This method gives access to the full Argus Presentation compiler. This should be used by
   *  internal code only.
   *
   ...
   */
  private[arguside] def internal[U](op: ArgusPresentationCompiler => U): Option[U] = {
    def obtainPc(): ArgusPresentationCompiler = {
   ...
   */
  private def create(): ArgusPresentationCompiler = {
    pcLock.synchronized {
      try {
        val pc = new ArgusPresentationCompiler(name, initializeSettings())
