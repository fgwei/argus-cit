...
/** An implementation of position information that is based on a Argus SourceFile implementation
 */
...
class PlainArgusInfo(file: AbstractFile, override val originalSource: Array[Char]) extends ISourceMap {
  ...
  override val argusPos = IPositionInformation.plainArgus(sourceFile)
  override val originalPos = IPositionInformation.plainArgus(sourceFile)
