...
import org.arguside.core.completion.CompletionContext
import org.arguside.core.internal.jdt.search.ArgusIndexBuilder
import org.arguside.core.internal.jdt.model.ArgusJavaMapper
import org.arguside.core.internal.jdt.search.ArgusMatchLocator
import org.arguside.core.internal.jdt.model.ArgusStructureBuilder
import org.arguside.ui.internal.jdt.model.ArgusOverrideIndicatorBuilder
...
import org.arguside.core.completion.CompletionProposal
...
import org.arguside.core.compiler._
import org.arguside.core.compiler.IArgusPresentationCompiler._
import org.arguside.ui.internal.editor.hover.ArgusDocHtmlProducer
...
class ArgusPresentationCompiler(name: String, _settings: Settings) extends {
  /*
...
} with Global(_settings, new ArgusPresentationCompiler.PresentationReporter, name)
  with ArgusdocGlobalCompatibilityTrait
  with ArgusStructureBuilder
  with ArgusIndexBuilder
  with ArgusMatchLocator
  with ArgusOverrideIndicatorBuilder
  with ArgusJavaMapper
  with JavaSig
  with LocateSymbol
  with CompilerApiExtensions
  with IArgusPresentationCompiler
  with HasLogger
  with Argusdoc { self =>

  override lazy val analyzer = new {
    val global: ArgusPresentationCompiler.this.type = ArgusPresentationCompiler.this
  } with InteractiveArgusdocAnalyzer with CommentPreservingTypers {
  ...
  override def forArgusdoc = true

  def presentationReporter = reporter.asInstanceOf[ArgusPresentationCompiler.PresentationReporter]
  ...
  def problemsOf(file: AbstractFile): List[ArgusCompilationProblem] = {
  ...
  def problemsOf(scu: InteractiveCompilationUnit): List[ArgusCompilationProblem] = problemsOf(scu.file)
  ...
  def loadedType(sourceFile: SourceFile, keepLoaded: Boolean = false): Either[Tree, Throwable] = {
    if (self.onCompilerThread)
      throw ArgusPresentationCompiler.InvalidThread("Tried to execute `askLoadedType` while inside `ask`")
  ...
  def mkCompletionProposal(prefix: String, start: Int, sym: Symbol, tpe: Type,
    inherited: Boolean, viaView: Symbol, context: CompletionContext.ContextType, project: IArgusProject): CompletionProposal = {
    ...
    def addBackTicksIfNecessary(identifier: String): String = {
      def needsBackTicks(identifier: String) = {
        ...
        try {
          val tokens = ArgusLexer.tokenise(identifier)  // The last token is always EOF
          tokens.size match {
            case 1 => true    // whitespace
            case 2 => !(IDS contains tokens.head.tokenType)
            case more => true
          }
        } catch {case _: ArgusLexerException => true  /* Illegal chars encountered */}
      }
    ...
    val docFun = () => {
      ...
      if (comment.isDefined) (new ArgusDocHtmlProducer).getBrowserInput(this)(comment.get, sym, header.getOrElse("")) else None
    }
    ...
    asyncExec {
      findDeclaration(sym, javaProject) map {
        case (f, pos) =>
          ...
          new ArgusHyperlink(openableOrUnit = f,
              region = targetRegion,
              label = label(sym),
              text = name,
              wordRegion = region)
...
object ArgusPresentationCompiler {
  case class InvalidThread(msg: String) extends RuntimeException(msg)

  def defaultArgusSettings(errorFn: String => Unit = Console.println): Settings = new Settings(errorFn)

  class PresentationReporter extends InteractiveReporter {
    var compiler: ArgusPresentationCompiler = null
    ...
    def eclipseProblem(prob: Problem): Option[ArgusCompilationProblem] = {
      import prob._
      if (pos.isDefined) {
        ...
        val reducedPos: Option[Position]=
          if (pos.isRange)
            ...
            val wordPos = Try(PilarWordFinder.findWord(source.content, pos.start).getLength).toOption
        ...
        reducedPos flatMap { reducedPos =>
          ...
          fileName.map(ArgusCompilationProblem(
            _,
