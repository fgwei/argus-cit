...
import org.arguside.core.internal.jdt.search.ArgusSelectionEngine
import org.arguside.core.internal.jdt.search.ArgusSelectionRequestor
...
class DeclarationHyperlinkDetector extends BaseHyperlinkDetector with HasLogger {
  ...
  protected val resolver: ArgusDeclarationHyperlinkComputer = new ArgusDeclarationHyperlinkComputer
  ...
  override protected def runDetectionStrategy(icu: InteractiveCompilationUnit, textEditor: ITextEditor, currentSelection: IRegion): List[IHyperlink] = {
    ...
    val wordRegion = PilarWordFinder.findWord(doc.get, currentSelection.getOffset)
  ...
  protected def findHyperlinks(textEditor: ITextEditor, icu: InteractiveCompilationUnit, wordRegion: IRegion): List[IHyperlink] = {
    findHyperlinks(textEditor, icu, wordRegion, wordRegion.translate(icu.lastSourceMap().argusPos))
  }
      ...
      case Some(List()) =>
        icu match {
          case icuOpenable: InteractiveCompilationUnit with Openable =>
            ...
            // ArgusSelectionEngine. However, this is a last-resort hyperlinking that uses search for
            // top-level types, and unless there are bugs, normal hyperlinking (through compiler symbols)
            // would find it. So we go here only for `ArgusCompilationUnit`s.
...
object JavaSelectionEngine extends HasLogger {
 protected[core] def getJavaElements(icu: InteractiveCompilationUnit, openable: Openable, mappedRegion: IRegion): List[IJavaElement] = {
    try {
      ...
      val requestor = new ArgusSelectionRequestor(environment.nameLookup, openable)
      val engine = new ArgusSelectionEngine(environment, requestor, icu.argusProject.javaProject.getOptions(true))
