 ...
 *
 *  The Argus compiler is a managed resource. It may be shutdown when the classpath changes, etc.
 *
...
trait IPresentationCompilerProxy {
  ...
  def foreach(f: IArgusPresentationCompiler => Unit): Unit = {
    apply(f)
  }
   ...
   */
  def apply[U](op: IArgusPresentationCompiler => U): Option[U]
