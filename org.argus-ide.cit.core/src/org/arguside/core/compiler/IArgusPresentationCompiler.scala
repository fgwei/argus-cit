...
/** This interface provides access to Argus Presentation compiler services. Even though methods are inherited from
 *  `scala.tools.nsc.interactive.Global`, prefer the convenience methods offered in this trait.
 *
 *  The presentation compiler is an asynchronous layer on top of the Argus type-checker. The PC
 ...
 *    - unloaded (unmanaged). The PC won't attempt to parse or type-check it. Usually, any Argus file that is not
 ...
 */
trait IArgusPresentationCompiler extends Global with CompilerApiExtensions with InternalCompilerServices { self: ArgusPresentationCompiler =>
  import IArgusPresentationCompiler._
   ...
   */
  def problemsOf(scu: InteractiveCompilationUnit): List[ArgusCompilationProblem]

  /** Find the definition of given symbol. Returns a compilation unit and an offset in that unit.
   *
   *  @note The offset is relative to the Argus source file represented by the given unit. This may
   *        be different from the absolute offset in the workspace file of that unit if the unit is
   *        not a Argus source file. For example, Play templates are translated on the fly to Argus
   *        sources. The returned offset would be relative to the Argus translation, and would need
  ...
  /** Return the JDT element corresponding to this Argus symbol. This method is time-consuming
   *  and may trigger building the structure of many Argus files.
   *
  ...
  /** Return the compilation unit (Argus File) in which the symbol passed as an argument is defined.
   *  This method is time-consuming and may trigger building the structure of many Argus files.
   *
  ...
  /** Return the parsed scaladoc comment available at the definition of a Argus symbol.
   *
  ...
  /** Create a Argus CompletionProposal. This method is the exit point from the compiler cake,
  ...
  def mkCompletionProposal(prefix: String,
    ...
    project: IArgusProject): CompletionProposal
...
object IArgusPresentationCompiler extends HasLogger {
  /** The maximum time to wait for an `askOption` call to finish. */
  final val AskTimeout: Duration = if (IArgusPlugin().noTimeoutMode) Duration.Inf else 10000.millis
  ...
  object Implicits {
    implicit class RichResponse[A](val resp: Response[A]) extends AnyVal {
      ...
      def getOption(timeout: Duration = AskTimeout): Option[A] = {
        val res = if (IArgusPlugin().noTimeoutMode) Some(resp.get) else resp.get(timeout.toMillis)
