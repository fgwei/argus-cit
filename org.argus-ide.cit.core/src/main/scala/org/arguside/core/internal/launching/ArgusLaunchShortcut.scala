...
import org.arguside.util.eclipse.EclipseUtils.RichAdaptable
 ...
 * configurations.
 */
class ArgusLaunchShortcut extends JavaLaunchShortcut {
  ...
  override def findTypes(elements: Array[AnyRef], context: IRunnableContext): Array[IType] = {
    if (elements == null || elements.isEmpty)
      ...
      elements.flatMap(ArgusLaunchShortcut.getMainMethods).toArray
    }
  }
  ...
  override def getConfigurationType =
    getLaunchManager.getLaunchConfigurationType("argus.application")
...
object ArgusLaunchShortcut {

  /** Return all classes that contain @Test methods. */
  /* ZW: to make things simple, don't consider Junit right now
  def getJunitTestClasses(element: AnyRef): List[IType] = {
    ...
    je.getOpenable match {
      case scu: JawaSourceFile => JUnit4TestFinder.findTestClasses(scu)
      case _                    => Nil
    }
  }
  * 
  */
  ...
  def getMainMethods(element: AnyRef): List[IType] = {
    (for {
      ...
      case scu: JawaSourceFile =>

        scu.withSourceFile { (source, comp) =>
          Nil
          /* W.Zhou: to comment this out for temporarily pass the first build
          import comp._
          import definitions._

          def isTopLevelModule(cdef: definitions.Tree) = comp.asyncExec {
