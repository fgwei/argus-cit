...
/** Holds a reference to a 'live' presentation compiler and manages its lifecycle.
  ...
  */
final class PresentationCompilerProxy(name: String) extends IPresentationCompilerProxy
  ...
  /** Current 'live' instance of the presentation compiler.
    ...
    */
  private var pc: JawaPresentationCompiler = null
  ...
  override def apply[U](op: IJawaPresentationCompiler => U): Option[U] =
    internal(op)

  /** This method gives access to the full Jawa Presentation compiler. This should be used by
   *  internal code only.
   *
   ...
   */
  private[arguside] def internal[U](op: JawaPresentationCompiler => U): Option[U] = {
    def obtainPc(): JawaPresentationCompiler = {
   ...
   */
  private def create(): JawaPresentationCompiler = {
    pcLock.synchronized {
      try {
        val pc = new JawaPresentationCompiler(name)
