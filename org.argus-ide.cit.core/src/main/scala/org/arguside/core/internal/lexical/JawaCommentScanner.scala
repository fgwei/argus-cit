 ...
 * which can't be directly used because it doesn't extend [[AbstractJawaScanner]].
 * It was also not possible to extend `JavaCommentScanner` directly because it loads
 * all color properties from JDT and not from CIT.
 *
 * This class now uses all color properties from SDT but loads the task tags from JDT.
 * This means that all task tags configured for the Java editor also work for the Jawa
