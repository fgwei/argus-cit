...
import org.arguside.core.internal.launching.ArgusLaunchShortcut
import org.arguside.core.internal.launching

class ArgusLaunchableTester extends PropertyTester {
  /**
  ...
  private def hasMain(element: IJavaElement): Boolean = {
    try {
      ArgusLaunchShortcut.getMainMethods(element).nonEmpty
    } catch {
