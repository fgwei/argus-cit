...
import org.arguside.ui.editor.JawaEditorAnnotation
...
object QuickAssistProcessor {
   ...
   */
  final val DefaultId = "org.arguside.core.quickassist"
}

/**
 * A quick assist processor that executes quick assists that are registered
 * for the jawa editor. Always returns at least one element when
...
final class QuickAssistProcessor(input: IEditorInput, id: String) extends IQuickAssistProcessor {
  ...
  override def computeQuickAssistProposals(ctx: IQuickAssistInvocationContext): Array[ICompletionProposal] = {
    ...
    def assistLocations = {
      ...
      val model = ArgusPlugin().documentProvider.getAnnotationModel(input)
      ...
      def loop(as: Assists, fp: FirstProblem): (Assists, FirstProblem) =
        if (iter.isEmpty)
          ...
          case a: Annotation if a.isInstanceOf[JawaEditorAnnotation] || a.isInstanceOf[IJavaAnnotation] =>
            val (start, end) = {
    ...
    val ssf = IArgusPlugin().jawaCompilationUnit(input)
    val quickAssists =
      if (id == DefaultId)
        QuickAssists
      else
