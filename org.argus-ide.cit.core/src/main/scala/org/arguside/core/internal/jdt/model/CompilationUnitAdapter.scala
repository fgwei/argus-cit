...
class CompilationUnitAdapter(classFile: JawaClassFile) extends Openable(classFile.getParent.asInstanceOf[JavaElement]) with ICompilationUnit with env.ICompilationUnit {
  ...
  override def getAdapter[T](adapter : Class[T]) : T = {
    val a = (classFile : IAdaptable).getAdapter(adapter)
    a.asInstanceOf[T]
  }
  ...
  def getPackageDeclarations(): Array[IPackageDeclaration] = Array(new CompilationUnitAdapter.ArgusPackageDeclaration(classFile.getPackage))
...
object CompilationUnitAdapter {
  ...
  private class ArgusPackageDeclaration(_package: PackageFragment) extends IPackageDeclaration {
