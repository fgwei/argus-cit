...
import org.arguside.core.internal.decorators.semantichighlighting.classifier.SymbolInfo
...
class Position (
  offset: Int,
  length: Int,
  val kind: SymbolTypes.SymbolType) extends TextPosition(offset, length) {
...
object Position {
  ...
  def from(symbolInfos: List[SymbolInfo]): List[Position] = {
    ...
    } yield new Position(region.getOffset, region.getLength, symbolType))
  }
}
