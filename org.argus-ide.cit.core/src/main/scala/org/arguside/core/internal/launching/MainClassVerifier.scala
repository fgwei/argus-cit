...
import org.arguside.ui.internal.handlers.CompilerLaunchErrorHandler
...
class MainClassVerifier {
   ...
   */
  def execute(project: IArgusProject, mainTypeName: String, hasBuildErrors: Boolean): IStatus = {
    canRunMain(project, mainTypeName, hasBuildErrors)
  }

  private def canRunMain(project: IArgusProject, mainTypeName: String, hasBuildErrors: Boolean): IStatus = {
    ...
    if (mainClass.nonEmpty && mainModuleClass.isEmpty) new Status(IStatus.ERROR, CitConstants.PluginId, s"${mainTypeName} needs to be an `object` (it is currently a `class`).")
    else if (hasBuildErrors) {
      val msg = s"Project ${project.underlying.getName} contains build errors."
      new Status(IStatus.ERROR, CitConstants.PluginId, CompilerLaunchErrorHandler.STATUS_CODE_LAUNCH_ERROR, msg, null)
    }
    else new Status(IStatus.OK, CitConstants.PluginId, "")
  }

  private def findClassFile(project: IArgusProject, mainTypeName: String): Option[IResource] = {
