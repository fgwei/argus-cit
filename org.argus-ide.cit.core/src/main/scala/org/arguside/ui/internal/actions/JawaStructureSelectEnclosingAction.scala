...
/**
 * A Jawa-aware replacement for {@link org.eclipse.jdt.internal.ui.javaeditor.selectionactions.StructureSelectEnclosingAction}.
 */
class JawaStructureSelectEnclosingAction(editor: JawaSourceFileEditor, selectionHistory: SelectionHistory) extends Action {
  ...
  override def run() {
    ...
    val astSelector = previousAstSelector match {
      ...
      case _ =>
        ...
        catch { case _: JawaParserException => return }
    }
