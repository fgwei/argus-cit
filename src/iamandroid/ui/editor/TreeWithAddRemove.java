 ...
 */
package iamandroid.ui.editor;

import iamandroid.AmandroidPlugin;
import iamandroid.AmandroidStructureConfigHelpers;
import iamandroid.preferences.FileTypesPreferencesPage;
import iamandroid.ui.dialogs.MapOfStringsInputDialog;
...
public abstract class TreeWithAddRemove extends Composite {
    ...
    private String getPathAsString(IPath p, IProject project) {
        ...
        return AmandroidStructureConfigHelpers.convertToProjectRelativePath(project.getFullPath().toString(), ret);
    }
    ...
    private void addTreeItem(String pathAsString) {
        if (pathAsString != null && pathAsString.trim().length() > 0) {
            ...
            item.setImage(AmandroidPlugin.getImageCache().get(getImageConstant()));
        }
    }
