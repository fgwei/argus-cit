...
import iamandroid.utils.ComboFieldEditor;
...
public class PilarCodeFormatterPage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
    ...
    private void updateLabelExample(FormatStd formatStd) {
        ...
        Tuple<String, StyleRange[]> result = formatAndStyleRangeHelper.formatAndGetStyleRanges(formatStd, str,
                AmandroidPrefs.getChainedPrefStore(), true);
    ...
    public static boolean getFormatBeforeSaving() {
        return AmandroidPrefs.getPreferences().getBoolean(FORMAT_BEFORE_SAVING);
    }

    public static boolean getAutoformatOnlyWorkspaceFiles() {
        return AmandroidPrefs.getPreferences().getBoolean(AUTO_FORMAT_ONLY_WORKSPACE_FILES);
    }

    public static boolean getFormatOnlyChangedLines() {
        return AmandroidPrefs.getPreferences().getBoolean(FORMAT_ONLY_CHANGED_LINES);
    }

    public static boolean getAddNewLineAtEndOfFile() {
        return AmandroidPrefs.getPreferences().getBoolean(ADD_NEW_LINE_AT_END_OF_FILE);
    }

    public static boolean getTrimLines() {
        return AmandroidPrefs.getPreferences().getBoolean(TRIM_LINES);
    }

    public static boolean getTrimMultilineLiterals() {
        return AmandroidPrefs.getPreferences().getBoolean(TRIM_MULTILINE_LITERALS);
    }

    public static boolean useSpaceAfterComma() {
        return AmandroidPrefs.getPreferences().getBoolean(USE_SPACE_AFTER_COMMA);
    }

    public static boolean useSpaceForParentesis() {
        return AmandroidPrefs.getPreferences().getBoolean(USE_SPACE_FOR_PARENTESIS);
    }

    public static boolean useAssignWithSpacesInsideParenthesis() {
        return AmandroidPrefs.getPreferences().getBoolean(USE_ASSIGN_WITH_PACES_INSIDER_PARENTESIS);
    }

    public static boolean useOperatorsWithSpace() {
        return AmandroidPrefs.getPreferences().getBoolean(USE_OPERATORS_WITH_SPACE);
    }

    public static int getSpacesBeforeComment() {
        int spaces = AmandroidPrefs.getPreferences().getInt(SPACES_BEFORE_COMMENT);
    ...
    public static int getSpacesInStartComment() {
        int spaces = AmandroidPrefs.getPreferences().getInt(SPACES_IN_START_COMMENT);
        if (spaces < FormatStd.DONT_HANDLE_SPACES) {
            spaces = FormatStd.DONT_HANDLE_SPACES;
        }
        return spaces;
