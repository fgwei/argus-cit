 ...
 */
public abstract class PilarAction extends Action implements IEditorActionDelegate {

    protected PilarAction() {
        super();
    }

    protected PilarAction(String text, int style) {
        super(text, style);
    }
    ...
    public static String getDelimiter(IDocument doc) {
        return PilarSelection.getDelimiter(doc);
    }
     ...
     */
    protected PilarEditor getPilarEditor() {
        if (targetEditor instanceof PilarEditor) {
            return (PilarEditor) targetEditor;
        } else {
    ...
    protected void gotoFirstVisibleChar(IDocument doc, int cursorOffset) {
        try {
            setCaretPosition(PilarSelection.getFirstCharPosition(doc, cursorOffset));
        } catch (BadLocationException e) {
            beep(e);
        }
    }
    ...
    protected boolean isAtFirstVisibleChar(IDocument doc, int cursorOffset) {
        try {
            return PilarSelection.getFirstCharPosition(doc, cursorOffset) == cursorOffset;
        } catch (BadLocationException e) {
            return false;
        }
    }
     ...
     * 
     */
    public static String getLineWithoutComments(PilarSelection ps) {
     ...
     */
    protected void revealSelEndLine(PilarSelection ps) {
...
//        IWorkbenchWindow activeWorkbenchWindow = PilarEditor.getActiveWorkbenchWindow();
...
//                if (!PilarEditor.EDITOR_ID.equals(iEditorReference.getId())) {
