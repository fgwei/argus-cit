...
import amanide.ui.dialogs.MapOfStringsInputDialog;
...
public abstract class TreeWithAddRemove extends Composite {
	...
	/**
	 * @param initialItems
	 *            : Can be a String[] or a HashMap<String, String> (if null it's
	 *            considered String[])
	 */
	public TreeWithAddRemove(Composite parent, int style, Object initialItems,
			boolean createEditButton) {
		super(parent, style);
		...
		buttonsSourceFolders.setLayoutData(new GridData(
		...
		if (initialItems instanceof String[]) {
			...
			throw new RuntimeException("Unexpected initial items: "
					+ initialItems);
		}
	...
	protected void createAddButton(Composite buttonsSourceFolders,
			int buttonNumber) {
		Button buttonAddSourceFolder = new Button(buttonsSourceFolders,
				SWT.PUSH);
	...
	protected void createRemoveButton(Composite buttonsSourceFolders) {
		Button buttonRemSourceFolder = new Button(buttonsSourceFolders,
				SWT.PUSH);
	...
	/**
	 * @param addButton
	 *            the button to be customized
	 * @param nButton
	 *            the number of the add button (1 for first, 2 for second)
	 */
	protected void customizeAddSomethingButton(Button addButton,
			final int nButton) {
	...
	public void addItemWithDialog(MapOfStringsInputDialog dialog) {
		if (dialog.open() == Window.OK) {
			Tuple<String, String> keyAndValueEntered = dialog
					.getKeyAndValueEntered();
		...
		if (objects != null) {
			for (int i = 0; i < objects.length; i++) {
				...
				if (object instanceof IPath) {
					...
					// IMPORTANT: get it relative to the workspace root, and not
					// to the project!!
					// (historical reasons)
					...
					String pathAsString = getPathAsString(
							p.getProjectRelativePath(), project);
					pathAsString = "/" + p.getProject().getName()
							+ pathAsString;
				}
			}
		}
	...
	/**
	 * @param project
	 * @return The passed path as a string (used for the selection dialog, as
	 *         things come relative to the workspace).
	 */
	private String getPathAsString(IPath p, IProject project) {
		...
		return AmanIDEStructureConfigHelpers.convertToProjectRelativePath(
				project.getFullPath().toString(), ret);
	}
	...
	private void addTreeItem(String pathAsString) {
		if (pathAsString != null && pathAsString.trim().length() > 0) {
			...
			item.setImage(AmanIDEPlugin.getImageCache().get(getImageConstant()));
		}
	}
	...
	public String getTreeItemsAsStr() {
		if (editingStyle != EDITING_STYLE_ARRAY_OF_STRINGS) {
			throw new AssertionError(
					"Can only call it if we're dealing with array of strings.");
		}
	...
	public Map<String, String> getTreeItemsAsMap() {
		if (editingStyle != EDITING_STYLE_MAP_OF_STRINGS) {
			throw new AssertionError(
					"Can only call it if we're dealing with map of strings.");
		}
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (text0 != null && text0.trim().length() > 0 && text1 != null
					&& text1.trim().length() > 0) {
	...
	// Things to customize (remove has a default implementation because it's
	// usually the same)
	...
	/**
	 * If a subclass specified that edit should be treated, it should also
	 * implement the handleEdit.
	 */
