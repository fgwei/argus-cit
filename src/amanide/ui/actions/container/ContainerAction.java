...
import amanide.utils.AsynchronousProgressMonitorDialog;
 ...
 * @author Fabio
 */
public abstract class ContainerAction implements IObjectActionDelegate {

	/**
	 * Subclasses can override to determine if the container should be refreshed
	 * before the action is executed or not.
	 */
	...
	public void selectionChanged(IAction action, ISelection selection) {
		...
		for (Iterator<Object> it = selections.iterator(); it.hasNext();) {
			...
			if (o instanceof IContainer) {
				...
				IContainer container = (IContainer) adaptable
	...
	/**
	 * Act on the selection to do the needed action (will confirm and make a
		...
		try {
			IRunnableWithProgress operation = new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor)
					...
					for (Iterator<IContainer> iter = selectedContainers
							.iterator(); iter.hasNext();) {
						IContainer next = iter.next();
						if (getRefreshBeforeExecute()) {
							// as files are generated externally, if we don't
							// refresh, it's very likely that we won't delete a
							// bunch of files.
							try {
								next.refreshLocal(IResource.DEPTH_INFINITE,
										monitor);
							} catch (Exception e) {
								Log.log(e);
							}
	 ...
	 * 
	 * @return true if UI access is needed (and false -- which is the default --
	 *         otherwise).
	 * 
	 * @note If it needs the UI access, it needs to call
	 *       Display.readAndDispatch() to assure that the interface remains
	 *       responsive.
	 */
	...
	/**
	 * Hook for clients to implement after the run is done (useful to show
	 * message)
	 * 
	 * @param resourcesAffected
	 *            the number of resources that've been affected.
	 */
	 ...
	 * 
	 * @param next
	 *            the container where the action should be executed
	 * @param monitor
	 *            The monitor that should be used to report the progress.
