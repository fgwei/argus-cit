...
import amanide.editors.StyledTextForShowingCodeFactory;
import amanide.editors.actions.PilarFormatStd;
import amanide.editors.actions.PilarFormatStd.FormatStd;
import amanide.utils.ComboFieldEditor;
import amanide.utils.Tuple;

/**
 * @author Fabio Zadrozny
 */
public class PilarCodeFormatterPage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
    ...
    public void createFieldEditors() {
        ...
        updateLabelExample(PilarFormatStd.getFormat());
    }
    ...
    private void updateLabelExample(FormatStd formatStd) {
        ...
        Tuple<String, StyleRange[]> result = formatAndStyleRangeHelper.formatAndGetStyleRanges(formatStd, str,
                AmanIDEPrefs.getChainedPrefStore(), true);
    ...
    public static boolean getFormatBeforeSaving() {
        return AmanIDEPrefs.getPreferences().getBoolean(FORMAT_BEFORE_SAVING);
    }

    public static boolean getAutoformatOnlyWorkspaceFiles() {
        return AmanIDEPrefs.getPreferences().getBoolean(AUTO_FORMAT_ONLY_WORKSPACE_FILES);
    }

    public static boolean getFormatOnlyChangedLines() {
        return AmanIDEPrefs.getPreferences().getBoolean(FORMAT_ONLY_CHANGED_LINES);
    }

    public static boolean getAddNewLineAtEndOfFile() {
        return AmanIDEPrefs.getPreferences().getBoolean(ADD_NEW_LINE_AT_END_OF_FILE);
    }

    public static boolean getTrimLines() {
        return AmanIDEPrefs.getPreferences().getBoolean(TRIM_LINES);
    }

    public static boolean getTrimMultilineLiterals() {
        return AmanIDEPrefs.getPreferences().getBoolean(TRIM_MULTILINE_LITERALS);
    }

    public static boolean useSpaceAfterComma() {
        return AmanIDEPrefs.getPreferences().getBoolean(USE_SPACE_AFTER_COMMA);
    }

    public static boolean useSpaceForParentesis() {
        return AmanIDEPrefs.getPreferences().getBoolean(USE_SPACE_FOR_PARENTESIS);
    }

    public static boolean useAssignWithSpacesInsideParenthesis() {
        return AmanIDEPrefs.getPreferences().getBoolean(USE_ASSIGN_WITH_PACES_INSIDER_PARENTESIS);
    }

    public static boolean useOperatorsWithSpace() {
        return AmanIDEPrefs.getPreferences().getBoolean(USE_OPERATORS_WITH_SPACE);
    }

    public static int getSpacesBeforeComment() {
        int spaces = AmanIDEPrefs.getPreferences().getInt(SPACES_BEFORE_COMMENT);
    ...
    public static int getSpacesInStartComment() {
        int spaces = AmanIDEPrefs.getPreferences().getInt(SPACES_IN_START_COMMENT);
        if (spaces < FormatStd.DONT_HANDLE_SPACES) {
            spaces = FormatStd.DONT_HANDLE_SPACES;
        }
        return spaces;
