...
/**
 * iterates through a string so that parts of it are gotten each time in a
 * progressive way based on dots
 * 
 * (e.g.: a.b.c
 * 
 * will return
 ...
 * a a.b a.b.c
 * 
 * copied from org.python.pydev.core.FullRepIterable.java
 */
public class FullRepIterable implements Iterable<String> {

	private static final class ReverseFullRepIterator implements
			Iterator<String> {
	...
	private static final class FullRepIterator implements Iterator<String> {
		private int i = -1;
		private boolean lastStep; // even if there is no point, we should return
									// the last string
		private String fullRep;
	 ...
	 * 
	 * @param fullRep
	 *            The dotted string we want to gather in parts
	 * @param reverse
	 *            whether we want to get it in reverse order
	 */
	...
	/**
	 * If 'aa' is passed, return String[]{'', 'aa'} If 'aa.bb' is passed, return
	 * String[]{'aa', 'bb'} If 'aa.bb.cc' is passed, return String[]{'aa.bb',
	 * 'cc'}
	 */
	 ...
	 * 
	 * head is pos 1 (cc) tail is pos 0 (aa.bb)
	 * 
	 * if it does not have a ".", everything is part of the tail - this is a
	 * strange behavior by the way... but it is what I need, because of the way
	 * to calculate the modules on some cases.
	 */
	public static String[] headAndTail(String fullRep, boolean emptyTailIfNoDot) {
		int i = fullRep.lastIndexOf('.');
		if (i != -1) {
			return new String[] { fullRep.substring(0, i),
	...
	/**
	 * @return the name of the parent module of the module represented by
	 *         currentModuleName
	 */
	...
	/**
	 * @return All that is after the last dot (or the whole string if there is
	 *         no dot)
	 */
	...
	/**
	 * @return All that is before the first dot (or the whole string if there is
	 *         no dot)
	 */
	...
	/**
	 * @return All that is before the last dot (or an empty string if there is
	 *         no dot)
	 */
