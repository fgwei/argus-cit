...
import amanide.editors.IPilarPartitions;
import amanide.editors.PilarSelection;

/**
 * Helper class for parsing pilar code.
 *
 * @author Fengguo Wei
 */
public abstract class ParsingUtils extends BaseParsingUtils implements IPilarPartitions {
    ...
    public static Iterator<String> getNoLiteralsOrCommentsIterator(IDocument doc) {
        return new PilarDocIterator(doc);
    }
        ...
        for (int i = 0; i < buf.length() && i < currPos; i++) {
            ...
            if (ch == '/') {
                  ...
                  i = j;
            			...
            			curr = PILAR_MULTILINE_COMMENT;
            			
            			int j = i;
                  while (j < buf.length() - 2 && !(ch == '*' && buf.charAt(j+1) == '/')) {
                      j++;
                      ch = buf.charAt(j);
                  }
                  i = j + 1;
            ...
            if (ch == '\'' || ch == '"') {
                ...
                if (multi) {
                    curr = PILAR_MULTILINE_STRING;
                } else {
                    curr = PILAR_SINGLELINE_STRING1;
                    if (ch == '"') {
                        curr = PILAR_SINGLELINE_STRING2;
                    }
                }
                try {
                    if (multi) {
                ...
                if (currPos == i) {
                    if (PILAR_SINGLELINE_STRING1.equals(curr) || PILAR_SINGLELINE_STRING2.equals(curr)) {
                        return curr;
                    }
                }
    ...
    public static String getContentType(IDocument document, int i) {
        ...
        IDocumentPartitionerExtension2 partitioner = (IDocumentPartitionerExtension2) docExtension
                .getDocumentPartitioner(IPilarPartitions.PILAR_PARTITION_TYPE);
    ...
    public static String makePilarParseable(String code, String delimiter) {
        return makePilarParseable(code, delimiter, new FastStringBuffer());
    }
     ...
     */
    public static String makePilarParseable(String code, String delimiter, FastStringBuffer lastLine) {
        ...
        for (int i = 0; i < code.length(); i++) {
            ...
            if (foundNewLineAtChar || i == code.length() - 1) {
                if (!PilarSelection.containsOnlyWhitespaces(currLine.toString())) {
...
//    }
    ...
    public static boolean isStringPartition(IDocument document, int offset) {
        ...
        return IPilarPartitions.PILAR_MULTILINE_STRING.equals(contentType)
                || IPilarPartitions.PILAR_SINGLELINE_STRING1.equals(contentType)
                || IPilarPartitions.PILAR_SINGLELINE_STRING2.equals(contentType);
    }

    public static boolean isCommentPartition(IDocument document, int offset) {
        String contentType = getContentType(document, offset);
        return IPilarPartitions.PILAR_SINGLELINE_COMMENT.equals(contentType)
        				|| IPilarPartitions.PILAR_MULTILINE_COMMENT.equals(contentType);
    }
