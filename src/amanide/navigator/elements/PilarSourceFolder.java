 ...
 * @author Fengguo Wei
 */
public class PilarSourceFolder implements IWrappedResource, IAdaptable,
	...
	public void addChild(IWrappedResource child) {
		...
		if (p != null && p instanceof IWrappedResource) {
			...
			if (pWrapped.getActualObject().equals(actualObject)) {
				Log.log("Trying to add an element that has itself as parent: "
						+ actualObject);
			}
		}

		// if there was already a child to the given object, remove it before
		// adding this one.
	...
	public String toString() {
		return "PilarSourceFolder [" + this.getActualObject() + "]";
	}
