...
import amanide.utils.DocIterator;
 ...
 */
public final class PilarSelection extends TextSelectionUtils {
     ...
     */
    public PilarSelection(ITextEditor textEditor) {
     ...
     */
    public PilarSelection(IDocument doc, ITextSelection selection) {
        super(doc, selection);
    }

    public static PilarSelection fromTextSelection(TextSelectionUtils ps) {
        return new PilarSelection(ps.getDoc(), ps.getTextSelection());
    }
     ...
     */
    public PilarSelection(IDocument doc, int line, int col) {
        this(doc, line, col, 0);
    }

    public PilarSelection(IDocument doc, int line, int col, int len) {
     ...
     */
    public PilarSelection(IDocument doc, int offset) {
     ...
     */
    public PilarSelection(IDocument doc) {
        this(doc, 0);
    }
     ...
     */
    public PilarSelection(PilarSelection base) {
...
//    }
    ...
    public String getLineContentsToCursor(boolean removeComments, boolean removeLiterals) throws BadLocationException {
        ...
        IDocumentPartitioner partitioner = PilarPartitionScanner.checkPartitionScanner(doc);
        if (partitioner == null) {
        ...
        for (int i = offset; i <= offset + length && i < cursorOffset; i++) {
            ...
            if (contentType.equals(IPilarPartitions.PILAR_DEFAULT)) {
...
//    }
    ...
    public LineStartingScope getPreviousLineThatStartsScope() {
        return getPreviousLineThatStartsScope(PilarSelection.INDENT_TOKENS, true, Integer.MAX_VALUE);
    }

    public LineStartingScope getNextLineThatStartsScope() {
        return getNextLineThatStartsScope(PilarSelection.INDENT_TOKENS, true, Integer.MAX_VALUE);
    }
    ...
    public LineStartingScope getPreviousLineThatStartsScope(int lineToStart) {
        return getPreviousLineThatStartsScope(PilarSelection.INDENT_TOKENS, lineToStart, Integer.MAX_VALUE);
    }
            ...
            for (String dedent : indentTokens) {
                if (trimmed.startsWith(dedent)) {
                    if (isCompleteToken(trimmed, dedent)) {
                        if (PilarSelection.getFirstCharPosition(line) < mustHaveIndentLowerThan) {
...
//    }
    ...
    public static boolean startsWithDedentToken(String trimmedLine) {
        for (String dedent : PilarSelection.DEDENT_TOKENS) {
    ...
    public static boolean startsWithIndentToken(String trimmedLine) {
        for (String dedent : PilarSelection.INDENT_TOKENS) {
            if (trimmedLine.startsWith(dedent)) {
                return isCompleteToken(trimmedLine, dedent);
            }
        }
