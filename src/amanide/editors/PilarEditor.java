...
public class PilarEditor extends TextEditor implements IPilarFormatStdProvider,
		IPilarSyntaxHighlightingAndCodeCompletionEditor {
	...
	// public final ICallbackWithListeners<PySourceViewer> onCreateSourceViewer
	// = new CallbackWithListeners<PySourceViewer>();
	...
	public PilarEditor() {
		// super();
		// colorCache = new
		// ColorAndStyleCache(AmandroidPrefs.getChainedPrefStore());
		// setSourceViewerConfiguration(new
		// PilarConfigurationWithoutEditor(colorCache));
		...
		try {
			...
			// editListeners =
			// ExtensionHelper.getParticipants(ExtensionHelper.PYDEV_PYEDIT_LISTENER);
			// }
			// notifier.notifyEditorCreated();
			colorCache = new ColorAndStyleCache(
					AmanIDEPrefs.getChainedPrefStore());

			editConfiguration = new PilarConfiguration(colorCache, this,
					AmanIDEPrefs.getChainedPrefStore());
			...
			// CodeFoldingSetter codeFoldingSetter = new
			// CodeFoldingSetter(this);
			// this.addModelListener(codeFoldingSetter);
			// this.addPropertyListener(codeFoldingSetter);

			// Don't show message anymore now that funding on indiegogo has
			// finished.
	...
	public void init(final IEditorSite site, final IEditorInput input)
		...
		try {
			...
			resetForceTabs();
			AmanIDEPrefs.getChainedPrefStore().addPropertyChangeListener(
					prefListener);
			...
			// public void run() {
			// try {
			// //let's do that in a thread, so that we don't have any delays in
			...
			public void propertyChange(PropertyChangeEvent event) {
				try {
					...
					if (property.equals(PilarEditorPrefs.TAB_WIDTH)) {
						ISourceViewer sourceViewer = editor
								.getEditorSourceViewer();
						if (sourceViewer == null) {
							return;
						}
						editor.getIndentPrefs().regenerateIndentString();
						sourceViewer.getTextWidget().setTabs(
								DefaultIndentPrefs.getStaticTabWidth());
						editor.resetIndentPrefixes();

					} else if (property
							.equals(PilarEditorPrefs.SUBSTITUTE_TABS)) {
						editor.getIndentPrefs().regenerateIndentString();
						editor.resetIndentPrefixes();

						// auto adjust for file tabs
					} else if (property
							.equals(PilarEditorPrefs.GUESS_TAB_SUBSTITUTION)) {
						editor.resetForceTabs();
						editor.resetIndentPrefixes();

						// colors and styles
					} else if (ColorAndStyleCache
							.isColorOrStyleProperty(property)) {
						editor.getColorCache().reloadProperty(property); // all
																			// reference
																			// this
																			// cache
						editor.getEditConfiguration()
								.updateSyntaxColorAndStyle(); // the style needs
																// no reloading
	...
	public Object getAdapter(@SuppressWarnings("rawtypes") Class adapter) {
		if (IContentOutlinePage.class.equals(adapter)) {
			if (outlinepage == null) {
				outlinepage = new PilarContentOutlinePage(
	...
	/**
	 * @param input
	 *            the input from where we want to get the document
	...
	public IProject getProject() {
		...
		if (editorInput instanceof FileEditorInput) {
			IFile file = (IFile) ((FileEditorInput) editorInput)
	...
	protected void initializeEditor() {
		...
		try {
			this.setPreferenceStore(AmanIDEPrefs.getChainedPrefStore());
	...
	/**
	 * Checks if there's a syntax error at the document... if there is, returns
	 * false.
	 * 
	 * Note: This function will also set the status line error message if
	 * there's an error message. Note: This function will actually do a parse
	 * operation when called (so, it should be called with care).
	 */
	public boolean hasSyntaxError(IDocument doc)
			throws MisconfigurationException {
		// Tuple2<Option<Model>, String> reparse =
		// PilarParser.parseWithErrorAsString(new Left<String,
		// String>(doc.get()), Model.class, 0);
