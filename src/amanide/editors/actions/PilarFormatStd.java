 ...
 */
package amanide.editors.actions;
...
import amanide.editors.PilarSelection;
import amanide.parser.IFormatter;
import amanide.preferences.PilarCodeFormatterPage;
import amanide.utils.FastStringBuffer;
import amanide.utils.Log;
import amanide.utils.ParsingUtils;
import amanide.utils.SelectionKeeper;
import amanide.utils.SyntaxErrorException;
import amanide.utils.TextSelectionUtils;
import amanide.utils.Tuple3;

/**
 * @author Fabio Zadrozny
 */
public class PilarFormatStd extends PilarAction implements IFormatter {
    ...
    public void run(IAction action) {
        try {
            ...
            PilarEditor pilarEditor = getPilarEditor();
            PilarSelection ps = new PilarSelection(pilarEditor);
            ...
            try {
                ...
                if (ps.getSelLength() > 0) {
                    ...
                    if (pilarEditor.hasSyntaxError(ps.getDoc())) {
                        return;
                    }
                }

                applyFormatAction(pilarEditor, ps, regionsToFormat, true, pilarEditor.getSelectionProvider());
            } catch (SyntaxErrorException e) {
                pilarEditor.getStatusLineManager().setErrorMessage(e.getMessage());
            }
     ...
     */
    public void applyFormatAction(IPilarFormatStdProvider pyEdit, PilarSelection ps, int[] regionsToFormat,
    ...
    public void formatSelection(IDocument doc, int[] regionsForSave, IPilarFormatStdProvider edit, PilarSelection ps) {
    ...
    public void formatSelection(IDocument doc, int[] regionsForSave, IPilarFormatStdProvider edit, PilarSelection ps,
        ...
        String delimiter = PilarSelection.getDelimiter(doc);
        IDocument formatted;
        try {
        ...
        try {
            for (int i : regionsForSave) {
                ...
                String line = PilarSelection.getLine(formatted, i);
            ...
            try {
                ...
                if (len > 0) {
                    ...
                    if (lastChar != '\r' && lastChar != '\n') {
                        doc.replace(len, 0, PilarSelection.getDelimiter(doc));
                    }
                }
            } catch (Throwable e) {
                Log.log(e);
     ...
     * @see IFormatter
     */
    public void formatAll(IDocument doc, IPilarFormatStdProvider edit, IFile f, boolean isOpenedFile,
            boolean throwSyntaxError)
            throws SyntaxErrorException {
        //        Formatter formatter = new Formatter();
    ...
    public void formatAll(IDocument doc, IPilarFormatStdProvider edit, boolean isOpenedFile, FormatStd formatStd,
    ...
    public static FormatStd getFormat() {
        ...
        formatStd.assignWithSpaceInsideParens = PilarCodeFormatterPage.useAssignWithSpacesInsideParenthesis();
        formatStd.operatorsWithSpace = PilarCodeFormatterPage.useOperatorsWithSpace();
        formatStd.parametersWithSpace = PilarCodeFormatterPage.useSpaceForParentesis();
        formatStd.spaceAfterComma = PilarCodeFormatterPage.useSpaceAfterComma();
        formatStd.addNewLineAtEndOfFile = PilarCodeFormatterPage.getAddNewLineAtEndOfFile();
        formatStd.trimLines = PilarCodeFormatterPage.getTrimLines();
        formatStd.trimMultilineLiterals = PilarCodeFormatterPage.getTrimMultilineLiterals();
        formatStd.spacesBeforeComment = PilarCodeFormatterPage.getSpacesBeforeComment();
        formatStd.spacesInStartComment = PilarCodeFormatterPage.getSpacesInStartComment();
        return formatStd;
    }
...
//            }
    ...
    private int handleOperator(FormatStd std, char[] cs, FastStringBuffer buf, ParsingUtils parsingUtils, int i, char c) {
        ...
        if (c == '~' || c == '+' || c == '-') {
            ...
            isUnary = trimmedLastWord.length() == 0 || PilarSelection.ALL_KEYWORD_TOKENS.contains(trimmedLastWord);
    ...
    private int formatForPar(final ParsingUtils parsingUtils, final char[] cs, final int i, final FormatStd std,
        ...
        if (c == ')') {
            ...
            String closing = ")";
            if (buf1.length() > 0 && PilarSelection.containsOnlyWhitespaces(buf1.toString())) {
